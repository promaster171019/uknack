...
class ProfileSerializer(BasicProfileSerializer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        from knacks.serializers import ProfileKnackSerializer
        from items.serializers import ProfileItemSerializer

        self.fields['knacks'] = ProfileKnackSerializer(many=True, read_only=True, depth=-1)
        self.fields['items'] = ProfileItemSerializer(many=True, read_only=True, depth=-1)
...
class KnackProfileSerializer(serializers.ModelSerializer):
    full_name = serializers.CharField(max_length=255, required=True, allow_blank=True)
    college = serializers.ReadOnlyField(source='college.name')
    year = serializers.ReadOnlyField(source='year.name')
    last_seen = serializers.SerializerMethodField()
    ...
    class Meta:
        model = User
        fields = ('username', 'full_name', 'first_name', 'age', 'college', 'picture', 'year', 'grand_total',
                  'created_at', 'last_seen', 'id', 'is_online', 'connections')

    def __init__(self, *args, depth=2, with_connections=True, **kwargs):
        if depth > 0 and with_connections:
            depth -= 1
            self.fields['connections'] = KnackProfileSerializer(depth=depth, with_connections=with_connections,
                                                                many=True, read_only=True)
        super(KnackProfileSerializer, self).__init__(*args, **kwargs)
