...
def fb_login(request):
    ...
    if backend_name:
        ...
        response = HttpResponseRedirect(settings.REDIRECT_URL +
                                        "/#/private-profile?social_code=" + code.code + "&social_backend=" +
                                        backend_name + "&token=" + token.key)
        response.set_cookie('social_code', code.code, domain=settings.COOKIE_DOMAIN)
        response.set_cookie('social_backend', backend_name, domain=settings.COOKIE_DOMAIN)
...
class CollegeFilter(django_filters.Filter):
    def filter(self, qs, value):
        if value not in (None, ''):
            values = [v for v in value.split(',')]
            return qs.filter(owner__college__name__in=values)
        return qs


class GenderFilter(django_filters.Filter):
    def filter(self, qs, value):
        if value not in (None, ''):
            values = [v for v in value.split(',')]
            return qs.filter(owner__gender__in=values)
        return qs


class YearFilter(django_filters.Filter):
    def filter(self, qs, value):
        if value not in (None, ''):
            values = [v for v in value.split(',')]
            return qs.filter(owner__year__name__in=values)
        return qs
...
class KnackFilter(django_filters.FilterSet):
    ...
    gender = GenderFilter(name="owner__gender", lookup_type='in')
    college = CollegeFilter(name="owner__college__name", lookup_type='in')
    year = YearFilter(name="owner__year__name", lookup_type='in')
